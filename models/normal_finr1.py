# -*- coding: utf-8 -*-
"""
Fin-R1 MCQ Evaluation - Normal version

Output columns:
- Pred_Choice:       model's predicted answer letter(s), normalized to [A-D]
- Why_Choice:        parsed "reason" field (with <think> stripped)
- Is_Correct:        bool, Pred_Choice == gold answer
- Confidence:        float in [0, 100]
- Raw_Model_Output:  full verbatim output including <think> for audit
"""
...
OUTPUT_DIR = os.path.abspath("./outputs_finr1")
...
def strip_think_tags(text: str) -> str:
    """
    Remove <think>...</think> blocks emitted by the reasoning model.
    Handles both properly-closed and (via the final branch) unclosed
    blocks caused by MAX_NEW_TOKENS truncation.
    """
    if not text:
        return text
    # Remove properly closed blocks
    text = re.sub(r'<think>.*?</think>', '', text, flags=re.DOTALL | re.IGNORECASE)
    # If a <think> is still present without a closing tag (truncated), drop from there on
    lower = text.lower()
...
def row_already_done(row) -> bool:
    """Done = non-empty Pred_Choice and non-ERROR Why_Choice."""
...
def build_prompt_chattemplated(tokenizer, question, a, b, c, d, is_chinese: bool):
    ...
    # Option separator: full-width colon for Chinese, half-width for English
    if is_chinese:
...
def parse_json_from_text(text: str):
    """
    Reasoning-model-aware JSON parser:
      (0) strip <think>...</think>
      (1) direct json.loads
...
def process_dataset(input_path: str, output_path: str, model, tokenizer, device):
    ...
    for idx in tqdm(range(len(df)), desc=f"  Fin-R1 ({lang})"):
        row = df.iloc[idx]
...
def main():
    print("=" * 60)
    print("Fin-R1 MCQ Evaluation (Normal)")
    ...
    # Fin-R1 is based on Qwen2.5-7B; standard kwargs are sufficient.
    # No need for legacy=True or use_fast=False.
    tokenizer = AutoTokenizer.from_pretrained(MODEL_DIR, trust_remote_code=True)

    if getattr(tokenizer, "chat_template", None) is None:
        raise RuntimeError(
            "Tokenizer chat_template is empty/missing. "
            "Fin-R1 (Qwen2.5-based) should have a built-in chat_template."
        )
