# -*- coding: utf-8 -*-
"""
DISC-FinLLM MCQ Evaluation - Masked (NOTA) version

Output columns:
- disc_finllm_reason
- disc_finllm_answer
- disc_finllm_confidence
- answer_from_reason
...
OUTPUT_DIR = os.path.abspath("./outputs_disc_finllm_masked")
...
def normalize_answerset(x: str) -> str:
    ...
    This avoids false-positive matches on incidental uppercase letters
    (e.g., 'ROE', 'GDP', 'CFA') appearing in natural-language text.
    """
    s = safe_str(x).upper()
...
def load_done_ids(output_path: str) -> set:
    """IDs whose model output has been successfully recorded (non-ERROR)."""
...
# =========================
# Answer extraction from reason (for judge_by_reason metric)
# =========================
def extract_answer_from_reason(reason: str, lang: str = "zh") -> str:
    ...
    reasoning text. Useful when the model writes the wrong letter in the
    'answer' field but reasons correctly in the 'reason' field.

    Patterns are tried in order of priority (conclusion-like > assertive >
    ...
    # Fallback: look at the last 200 chars, exclude enumerations like "A、B、C、D"
...
def chat_with_model(model, tokenizer, gen_config,
    ...
    """
    See docstring in normal_disc_finllm.py for Baichuan chat-template details.
    Determinism is enforced via explicit GenerationConfig with do_sample=False.
    """
    ...
    if hasattr(model, "chat"):
        try:
            return model.chat(tokenizer, messages, generation_config=gen_config)
        except TypeError:
...
def process_dataset(masked_path, original_path, lang,
    ...
    # Build ID -> original gold-answer map
    original_answers = {}
    ...
    # Enforce dtypes (avoid pandas auto-casting text columns to float)
    ...
    for idx in range(total):
        ...
        try:
            ...
            pred_answer, reason, conf, ok = parse_json_from_text(response)
            if not ok:
                pred_answer = ""
                conf = 0.0
                reason = "(parse failed)"
            ...
            err_msg = str(e)
            log(f"[ERROR] Row {idx} (ID={rid}): {err_msg[:150]}")
            df.at[idx, f"{MODEL_NAME}_reason"] = f"ERROR: {err_msg[:300]}"
...
def main():
    log("=" * 60)
    log("DISC-FinLLM MCQ Evaluation - Masked (NOTA)")
    ...
    for masked_path, original_path, lang in FILE_CONFIGS:
        ...
        process_dataset(masked_path, original_path, lang,
                        model, tokenizer, gen_config)
